...
class Trainer:
    def __init__(
        ...
        self.scaler = torch.cuda.amp.GradScaler(
            "cuda", enabled=self.enable_mixed_precision
        )
    ...
    def train(self) -> None:
        ...
        metrics, used_time = self.evaluator(self.model, "final_model")
        self.training_stats["eval_time"].update(used_time)
    ...
    def train_one_epoch(self, epoch: int) -> None:
        ...
        for batch_idx, data in enumerate(self.train_loader):
            ...
            # Weight decay schedule hack
            if self.schedule_wd:
                for param_group in self.optimizer.param_groups:
                    lr = param_group["weight_decay"] + 0.01
                    if isinstance(param_group["weight_decay"], torch.Tensor):
                        param_group["weight_decay"].fill_(lr)
                    else:
                        param_group["weight_decay"] = lr
    ...
    def save_best_checkpoint(
        ...
        if isinstance(curr_metric, list):
            curr_metric = (
                curr_metric[0] if self.num_classes == 1 else np.mean(curr_metric)
            )
        if self.best_metric_comp(curr_metric, self.best_metric):
            self.best_metric = curr_metric
            best_ckpt = self.get_checkpoint(epoch)
            self.save_model(epoch, is_best=True, checkpoint=best_ckpt)
    ...
    def log(self, batch_idx: int, epoch) -> None:
        ...
        left_eval_times = (
            (self.n_epochs - 0.5) // self.eval_interval
            + 2
